...
time_travel = pd.concat([time_travel.iloc[:3625]], ignore_index=True)
...
route_weekend = pd.read_csv("pi_w_go.csv")

time_travel = pd.concat([time_travel, route_weekend], axis=0)
time_travel.to_csv("pi_go.csv", index=False)
...
# train_time1 = pd.to_datetime("22:00:00", format="%H:%M:%S")
# train_data1 = []
...
#     [time_travel.iloc[:3596], pd.DataFrame(train_data1)], ignore_index=True
# )
...
#     ]
# ]
# route_weekend = pd.read_csv("pi_w_back.csv")

# time_travel = pd.concat([time_travel, route_weekend], axis=0)
# time_travel.to_csv("pi_back.csv", index=False)
