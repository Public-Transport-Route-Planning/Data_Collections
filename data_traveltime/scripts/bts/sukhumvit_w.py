        ...
        while current_time <= end_datetime:
            ...
            if journey < num_journeys - 1:
                if current_time >= pd.to_datetime(
                    "06:00:00", format="%H:%M:%S"  # time fix
                ) and current_time < pd.to_datetime("08:00:00", format="%H:%M:%S"):
                    service_interval = 7
                elif current_time >= pd.to_datetime(
                    "08:00:00", format="%H:%M:%S"
                ) and current_time < pd.to_datetime("11:00:00", format="%H:%M:%S"):
                    service_interval = 5.55
                elif current_time >= pd.to_datetime(
                    "11:00:00", format="%H:%M:%S"
                ) and current_time < pd.to_datetime("21:00:00", format="%H:%M:%S"):
                    service_interval = 4.30
                elif current_time >= pd.to_datetime(
...
station_route = "../../../station_route/data/bts/bts_su_route.csv"
...
# add the last train starting at 23:15:00 AM ##
train_time1 = pd.to_datetime("23:15:00", format="%H:%M:%S")
train_data1 = []
...
time_travel = pd.concat(
    [time_travel.iloc[:9752], pd.DataFrame(train_data1)], ignore_index=True
)
...
time_travel["route_id"] = "BTS-Sukhumvit"
...
time_travel.to_csv("su_w_go.csv", index=False)
...
#     ]
# ]
# time_travel.to_csv("su_w_back.csv", index=False)
