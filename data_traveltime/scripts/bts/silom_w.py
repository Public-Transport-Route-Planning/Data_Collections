...
# train_time1 = pd.to_datetime("23:58:00", format="%H:%M:%S") #23:36
# train_data1 = []
...
time_travel = pd.concat(
    [time_travel.iloc[:2431]], ignore_index=True
)

# time_travel = pd.concat(
#     [time_travel.iloc[:2392], pd.DataFrame(train_data1)], ignore_index=True
# )
...
for i in range(len(stations) - 1):
    ...
    time_travel.loc[time_travel["sid1"] == station1, "sname1"] = stations[i][1]
    time_travel.loc[time_travel["sid2"] == station2, "sname2"] = stations[i + 1][1]
...
time_travel.to_csv("si_w_go.csv", index=False)
