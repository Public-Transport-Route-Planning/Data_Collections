    ...
    for journey in range(num_journeys):
        ...
        while current_time <= end_datetime:
            ...
            if journey < num_journeys - 1:
                ...
                ) and current_time < pd.to_datetime("06:00:00", format="%H:%M:%S"):
                    service_interval = 9
...
travel_times = [5, 5, 4, 4, 5, 3, 2]  # add travel times for all stations

station_route = "../../../station_route/data/arl/arl_route.csv"
...
time_travel = pd.concat(
    [time_travel.iloc[:861], pd.DataFrame(train_data1)], ignore_index=True
)
...
route_weekend = pd.read_csv("arl_w_go.csv")

time_travel = pd.concat([time_travel, route_weekend], axis=0)
time_travel.to_csv("arl_go.csv", index=False)
...
# travel_times = [2, 3, 5, 4, 4, 5, 5]  # add travel times for all stations
...
# time_travel = pd.concat(
#     [time_travel.iloc[:868], pd.DataFrame(train_data1)], ignore_index=True
# )
...
# route_weekend = pd.read_csv("arl_w_back.csv")

# time_travel = pd.concat([time_travel, route_weekend], axis=0)
# time_travel.to_csv("arl_back.csv", index=False)
