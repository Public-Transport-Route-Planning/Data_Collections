...
#                 ) and current_time < pd.to_datetime("09:00:00", format="%H:%M:%S"):
#                     service_interval = 3.45
#                 elif current_time >= pd.to_datetime(
#                     "09:00:00", format="%H:%M:%S"
#                 ) and current_time < pd.to_datetime("09:30:00", format="%H:%M:%S"):
#                     service_interval = 6
#                 elif current_time >= pd.to_datetime(
#                     "09:30:00", format="%H:%M:%S"
#                 ) and current_time < pd.to_datetime("17:00:00", format="%H:%M:%S"):
#                     service_interval = 6
#                 elif current_time >= pd.to_datetime(
#                     "17:00:00", format="%H:%M:%S"
...
# station_route = "../../../station_route/data/bts/bts_si_route.csv"
...
# train_time1 = pd.to_datetime("23:58:00", format="%H:%M:%S")
# train_data1 = []
...
#     [time_travel.iloc[:5590], pd.DataFrame(train_data1)], ignore_index=True
# )
...
# time_travel["route_id"] = "BTS-Silom"
...
# route_weekend = pd.read_csv("si_w_go.csv")


# time_travel = pd.concat([time_travel, route_weekend], axis=0)
# time_travel.to_csv("si_go.csv", index=False)
...
# add the last train starting at 00:00 AM ##
train_time1 = pd.to_datetime("23:36:00", format="%H:%M:%S")
train_data1 = []
...
time_travel = pd.concat(
    [time_travel.iloc[:5538], pd.DataFrame(train_data1)], ignore_index=True
)
...
route_weekend = pd.read_csv("si_w_back.csv")

time_travel = pd.concat([time_travel, route_weekend], axis=0)
time_travel.to_csv("si_back.csv", index=False)
