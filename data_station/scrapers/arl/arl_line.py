...
url = "https://en.wikipedia.org/wiki/Airport_Rail_Link_(Bangkok)"
...
rows_to_drop = list(range(8, 10))
df = df[~df.index.isin(rows_to_drop)]

custom_order = [f"A{i:02}" for i in range(1, len(df) + 1)]  # code station
df["sid"] = custom_order  # new col

df = df[["sid", "name_e", "name_t"]]


target_directory = "../../data/airportlink"  # path output result
os.makedirs(target_directory, exist_ok=True)

csv_filename = "airport_station.csv"
