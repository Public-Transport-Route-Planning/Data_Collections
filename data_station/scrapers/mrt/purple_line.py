...
url = "https://en.wikipedia.org/wiki/Purple_Line_(Bangkok)"
...
rows_to_drop = list(range(0, 11)) + list(range(27, 44))
df = df[~df.index.isin(rows_to_drop)]

custom_order = [f"PP{i:02}" for i in range(1, len(df) + 1)]  # code station
...
csv_filename = "purple_station.csv"
