...
url = "https://en.wikipedia.org/wiki/Yellow_Line_(Bangkok)"
...
rows_to_drop = list(range(23, 25))
df = df[~df.index.isin(rows_to_drop)]

custom_order = [f"YL{i:02}" for i in range(1, len(df) + 1)]  # code station
...
csv_filename = "yellow_station.csv"
